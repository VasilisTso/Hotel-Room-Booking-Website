<?php 
    ...
    if(isset($_POST['add_room']))
    {
        ...
        $q1 = "INSERT INTO `rooms` (`name`, `area`, `price`, `quantity`, `adult`, `children`, `descr`) VALUES (?,?,?,?,?,?,?)";
        $values = [$frm_data['name'],$frm_data['area'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['descr']];
        ...
        while($row = mysqli_fetch_assoc($res))
        {
            ...
            $data.="
                <tr class='align-middle'>
                    ...
                    <td>$status</td>
                    <td>
                        <button type='button' onclick='edit_details($row[id])' class='btn btn-primary text-dark shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
                        ...
                        <button type='button' onclick='remove_room($row[id])' class='btn btn-danger text-dark shadow-none btn-sm'>
    ...
    if(isset($_POST['edit_room']))
    {
        ...
        $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`price`=?,`quantity`=?,`adult`=?,`children`=?,`descr`=? WHERE `id`=?";
        $values = [$frm_data['name'],$frm_data['area'],$frm_data['price'],$frm_data['quantity'],$frm_data['adult'],$frm_data['children'],$frm_data['descr'],$frm_data['room_id']];
    ...
    if(isset($_POST['get_room_images']))
    {
        ...
        while($row = mysqli_fetch_assoc($res)){
            if($row['thumb']==1){
                ...
                $thumb_btn = "<button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary text-dark shadow-none'>
            ...
            echo<<<data
                <tr class='align-middle'>
                    ...
                    <td>$thumb_btn</td>
                    <td>
                        <button onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-danger text-dark shadow-none'>
                            <i class="bi bi-trash-fill"></i>
                        </button> 
                    </td>
                </tr>
